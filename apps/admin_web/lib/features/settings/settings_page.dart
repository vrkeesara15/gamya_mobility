...
class _SettingsPageState extends ConsumerState<SettingsPage> {
  ...
  Widget build(BuildContext context) => PageBody(children: [
    ...
    if (_loading) const Card(child: LoadingState()) else if (_error != null) Card(child: ErrorState(message: _error!, onRetry: _load)) else [
      ...
      SectionCard(title: 'Tariffs (used for estimated amounts)', padding: const EdgeInsets.all(12), child: GTable<Map<String, dynamic>>(minWidth: 600, columns: const [GColumn('Vehicle Type', width: 160), GColumn('Base Amount', width: 120, numeric: true), GColumn('Per Km', width: 100, numeric: true), GColumn('Per Hour', width: 100, numeric: true), GColumn('Updated', width: 150), GColumn('Actions', flex: 1)], rows: _tariffs, cells: (t, i) => [CellText(Fmt.vehicleType(t['vehicleType'] as String?), bold: true), CellText(Fmt.inr(t['baseAmount'])), CellText(Fmt.inr(t['perKm'])), CellText(Fmt.inr(t['perHour'])), CellText(Fmt.date(t['updatedAt'])), OutlineButton(label: 'Edit', dense: true, icon: Icons.edit_outlined, onPressed: () => _saveTariff(t))])),
