    ...
    return PageBody(children: [
      ...
      if (_loading) const LoadingState() else if (_error != null) ErrorState(message: _error!, onRetry: _load) else ...[
        StatGrid(cards: [
          ...
          StatCard(label: 'Revenue', value: Fmt.inr(_summary['revenue']), icon: Icons.currency_rupee, trend: 'Completed trips', trendPositive: true),
          StatCard(label: 'Driver Payout', value: Fmt.inr(_summary['driverPayout']), icon: Icons.account_balance_wallet, trend: 'Earnings accrued', trendColor: GamyaColors.info, trendIcon: Icons.arrow_upward),
        ...
        Card(child: Padding(padding: const EdgeInsets.all(12), child: _tab == 0
          ? GTable<Map<String, dynamic>>(minWidth: 800, columns: const [GColumn('Driver', width: 180), GColumn('Mobile', width: 110), GColumn('Vehicle', width: 110), GColumn('Trips', width: 60, numeric: true), GColumn('Completed', width: 80, numeric: true), GColumn('Delayed', width: 70, numeric: true), GColumn('On-Time %', width: 80, numeric: true), GColumn('Earnings', width: 100, numeric: true), GColumn('Rating', width: 70, numeric: true)], rows: _drivers, cells: (d, i) => [CellText(d['fullName'] as String?, bold: true), CellText(d['mobile'] as String?), CellText(d['vehicleNumber'] as String?), CellText('${d['trips']}'), CellText('${d['completed']}'), CellText('${d['delayed']}', color: n(d['delayed']) > 0 ? GamyaColors.danger : null), CellText('${d['onTimePct']}%', color: n(d['onTimePct']) >= 90 ? GamyaColors.success : GamyaColors.warning), CellText(Fmt.inr(d['earnings']), bold: true), Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.star, size: 13, color: GamyaColors.gold), CellText('${d['rating'] ?? '—'}')])])
          : GTable<Map<String, dynamic>>(minWidth: 800, columns: const [GColumn('Vehicle', width: 120), GColumn('Make & Model', width: 160), GColumn('Type', width: 100), GColumn('Driver', width: 150), GColumn('Trips', width: 60, numeric: true), GColumn('Trips / Day', width: 80, numeric: true), GColumn('Distance (km)', width: 100, numeric: true), GColumn('Revenue', width: 100, numeric: true)], rows: _vehicles, cells: (v, i) => [CellText(v['number'] as String?, bold: true), CellText(v['makeModel'] as String?), CellText(Fmt.vehicleType(v['type'] as String?)), CellText(v['driverName'] as String?), CellText('${v['trips']}'), CellText('${v['tripsPerDay']}'), CellText('${v['distanceKm']}'), CellText(Fmt.inr(v['revenue']), bold: true)]))),
      ],
    ]);
  }
}
