...
class _PaymentsPageState extends ConsumerState<PaymentsPage> {
  ...
  Widget build(BuildContext context) {
    return PageBody(children: [
      ...
      StatGrid(cards: [
        StatCard(label: 'Pending Driver Payouts', value: Fmt.inr(_stats['pendingEarnings']), icon: Icons.account_balance_wallet, iconColor: GamyaColors.warning, trend: '${_stats['pendingEarningsCount'] ?? 0} trips unsettled', trendColor: GamyaColors.warning, trendIcon: Icons.schedule),
        StatCard(label: 'Settled Payouts', value: Fmt.inr(_stats['settledEarnings']), icon: Icons.check_circle, iconColor: GamyaColors.success, trend: '${_stats['paidSettlementsCount'] ?? 0} settlements paid', trendPositive: true),
        StatCard(label: 'Settlements Pending', value: Fmt.inr(_stats['pendingSettlements']), icon: Icons.pending_actions, trend: '${_stats['pendingSettlementsCount'] ?? 0} batches', trendColor: GamyaColors.info, trendIcon: Icons.layers),
        StatCard(label: 'Earnings This Month', value: Fmt.inr(_stats['monthEarnings']), icon: Icons.currency_rupee, trend: 'Driver earnings', trendPositive: true),
        StatCard(label: 'Invoices Due', value: Fmt.inr(_stats['invoicesDue']), icon: Icons.receipt_long, iconColor: GamyaColors.danger, trend: '${_stats['invoicesDueCount'] ?? 0} invoices', trendPositive: false),
        StatCard(label: 'Invoices Paid', value: Fmt.inr(_stats['invoicesPaid']), icon: Icons.paid, iconColor: GamyaColors.success, trend: '${_stats['invoicesPaidCount'] ?? 0} invoices', trendPositive: true),
      ]),
      ...
      if (_loading) const Card(child: LoadingState()) else if (_error != null) Card(child: ErrorState(message: _error!, onRetry: _load)) else Card(child: Padding(padding: const EdgeInsets.all(12), child: [
        GTable<Map<String, dynamic>>(minWidth: 700, columns: const [GColumn('Driver', width: 200), GColumn('Mobile', width: 110), GColumn('Trips', width: 70, numeric: true), GColumn('Pending Amount', width: 120, numeric: true), GColumn('Actions', flex: 1)], rows: _summary, emptyText: 'All driver earnings are settled', cells: (d, i) => [PersonCell(name: d['fullName'] as String? ?? '', avatarUrl: d['avatarUrl'] as String?, subtitle: d['code'] as String?), CellText(d['mobile'] as String?), CellText('${d['trips']}'), CellText(Fmt.inr(d['pendingAmount']), bold: true, color: GamyaColors.success), Row(children: [GoldButton(label: 'Create Settlement', dense: true, onPressed: () => _settle(d)), const SizedBox(width: 6), OutlineButton(label: 'View Driver', dense: true, onPressed: () => context.go('/drivers?id=${d['driverId']}'))])]),
        Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [GTable<Map<String, dynamic>>(minWidth: 800, columns: const [GColumn('Date', width: 110), GColumn('Driver', width: 160), GColumn('Trip', width: 120), GColumn('Route', flex: 2), GColumn('Platform', width: 110), GColumn('Amount', width: 90, numeric: true), GColumn('Status', width: 90), GColumn('Settlement', width: 140)], rows: _earnings.items, cells: (e, i) => [CellText(Fmt.date(e['date'])), CellText((e['driver'] as Map)['fullName'] as String?), CellText((e['trip'] as Map?)?['code'] as String?), (e['trip'] == null) ? const CellText('—') : RouteText((e['trip'] as Map)['fromLocation'] as String, (e['trip'] as Map)['toLocation'] as String), CellText((e['trip'] as Map?)?['platform'] as String?), CellText(Fmt.inr(e['amount']), bold: true), StatusChip(e['status'] as String?, small: true), CellText(e['settlementCode'] as String?, muted: true)]), const SizedBox(height: 10), PaginationBar(paged: _earnings, noun: 'earnings', onPage: (p) { _qe.page = p; _load(); })]),
        Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [GTable<Map<String, dynamic>>(minWidth: 800, columns: const [GColumn('Settlement', width: 150), GColumn('Driver', width: 170), GColumn('Period', width: 190), GColumn('Trips', width: 60, numeric: true), GColumn('Amount', width: 100, numeric: true), GColumn('Status', width: 90), GColumn('Reference', width: 130), GColumn('Actions', width: 120)], rows: _settlements.items, emptyText: 'No settlements yet', cells: (s, i) => [CellText(s['code'] as String?, bold: true), CellText((s['driver'] as Map)['fullName'] as String?), CellText('${Fmt.date(s['periodStart'])} – ${Fmt.date(s['periodEnd'])}'), CellText('${s['trips']}'), CellText(Fmt.inr(s['totalAmount']), bold: true), StatusChip(s['status'] as String?, small: true), CellText(s['reference'] as String? ?? (s['paidAt'] != null ? Fmt.date(s['paidAt']) : null), muted: true), s['status'] == 'PAID' ? const SizedBox() : GoldButton(label: 'Mark Paid', dense: true, onPressed: () => _pay(s))]), const SizedBox(height: 10), PaginationBar(paged: _settlements, noun: 'settlements', onPage: (p) { _qs.page = p; _load(); })]),
        Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [GTable<Map<String, dynamic>>(minWidth: 800, columns: const [GColumn('Invoice', width: 150), GColumn('Client', width: 150), GColumn('Period', width: 190), GColumn('Trips', width: 60, numeric: true), GColumn('Amount', width: 110, numeric: true), GColumn('Due Date', width: 100), GColumn('Status', width: 90), GColumn('Actions', width: 200)], rows: _invoices.items, emptyText: 'No invoices yet', cells: (v, i) => [CellText(v['code'] as String?, bold: true), CellText(v['clientName'] as String?), CellText('${Fmt.date(v['periodStart'])} – ${Fmt.date(v['periodEnd'])}'), CellText('${v['tripCount']}'), CellText(Fmt.inr(v['amount']), bold: true), CellText(Fmt.date(v['dueDate'])), StatusChip(v['status'] as String?, small: true), Row(children: [if (v['status'] == 'DRAFT') OutlineButton(label: 'Mark Sent', dense: true, onPressed: () => _invoiceStatus(v, 'SENT')), if (v['status'] != 'PAID') ...[const SizedBox(width: 6), GoldButton(label: 'Mark Paid', dense: true, onPressed: () => _invoiceStatus(v, 'PAID'))]])]), const SizedBox(height: 10), PaginationBar(paged: _invoices, noun: 'invoices', onPage: (p) { _qi.page = p; _load(); })]),
      ][_tab])),
    ]);
  }
}
