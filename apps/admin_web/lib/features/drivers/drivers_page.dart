      ...
      MasterDetail(
        master: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          CardRow(minWidth: 280, children: [
            SectionCard(title: 'Driver Performance (Last 30 Days)', child: Wrap(spacing: 8, runSpacing: 8, children: [SizedBox(width: 150, child: MiniStat(label: 'Total Trips', value: '${n(_perf['totalTrips'])}', icon: Icons.route)), SizedBox(width: 150, child: MiniStat(label: 'On Time', value: '${n(_perf['onTime'])} (${n(_perf['onTimePct'])}%)', icon: Icons.schedule)), SizedBox(width: 150, child: MiniStat(label: 'Delayed', value: '${n(_perf['delayed'])} (${n(_perf['delayedPct'])}%)', icon: Icons.block, color: GamyaColors.danger)), SizedBox(width: 150, child: MiniStat(label: 'Earnings', value: Fmt.inr(_perf['earnings']), icon: Icons.currency_rupee))])),
  ...
  Widget _detail(Driver d) {
    ...
    return DetailPanel(
      ...
      tabViews: [
        ...
        Column(children: [
          MiniStat(label: 'Earnings (last 30 days)', value: Fmt.inr(perf['earnings']), icon: Icons.currency_rupee), const SizedBox(height: 10),
          if (d.earnings.isEmpty) const EmptyState(title: 'No earnings yet') else for (final e in d.earnings.take(12)) ListTile(dense: true, contentPadding: EdgeInsets.zero, title: Text('${Fmt.date(e['date'])}  ·  ${(e['trip'] as Map?)?['code'] ?? ''}', style: const TextStyle(fontSize: 12.5)), subtitle: Text('${((e['trip'] as Map?)?['platform'] as Map?)?['name'] ?? 'Other'} · ${Fmt.title(e['status'] as String?)}', style: const TextStyle(fontSize: 11)), trailing: Text(Fmt.inr(e['amount']), style: const TextStyle(fontWeight: FontWeight.w700, color: GamyaColors.success))),
        ]),
