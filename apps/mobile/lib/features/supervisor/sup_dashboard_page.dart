  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(onRefresh: _load, child: ListView(padding: EdgeInsets.zero, children: [
        DashboardHeader(name: 'Hello, ${u.fullName}', avatarName: u.fullName, subtitle: '${_d['designation'] ?? 'Supervisor'} - ${_d['companyName'] ?? u.companyName ?? ''}', avatarUrl: _d['avatarUrl'] as String?, unread: n(_d['unreadNotifications']).toInt(), onMenu: () => _key.currentState?.openDrawer(), onBell: () => context.push('/notifications').then((_) => _load()), onAvatar: () => context.push('/profile')),
